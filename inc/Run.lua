...
terbo = Token:match("(%d+)")
redis:set(terbo..":VERSION",1)
redis:set(terbo..":SUDO_ID:",SUDO_USER)
redis:set(terbo..":DataCenter:",'German')
redis:set(terbo..":UserNameBot:",BOT_User)
redis:set(terbo..":NameBot:",BOT_NAME)
redis:hset(terbo..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3terbo_INSTALL","Yes")
info = {} 
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس تربو بنسبه 100% ]  }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE terbo source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JOKER : @eljo_ker_m  }•\e[0m"

echo -e "\e[38;5;77m      terbo ✾ @eljo_ker_me[0m"
...
terbo = Token:match("(%d+)")
our_id = tonumber(terbo)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(terbo..":UserNameBot:")
SUDO_ID = tonumber(redis:get(terbo..":SUDO_ID:"))
SUDO_USER = redis:hgetall(terbo..'username:'..SUDO_ID).username
version = redis:get(terbo..":VERSION")
DataCenter = redis:get(terbo..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..terbo..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(terbo..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(terbo..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(terbo..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 947425342 then 
msg.TheRank =  'مالك السورس'
msg.Rank = 1

elseif msg.sender_user_id_ == 1765160742 then 
...
msg.Rank = 1
elseif redis:sismember(terbo..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(terbo..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(terbo..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(terbo..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(terbo..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(terbo..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(terbo..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(terbo..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(terbo..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(terbo..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(terbo..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(terbo..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس  تربو ⇐ @ter_bo7. مـعرف المطور  :]]..SUDO_USER:gsub([[\_]],'_')..[[ .
]])
return false
end
if not msg then
...
if msg.GroupActive and redis:get(terbo..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(terbo..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(terbo..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(terbo..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.terbo then 
if msg.text and ScriptFile.iterbo then
for k, terbo in pairs(ScriptFile.terbo) do
local SearchText = msg.text:match(terbo)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iterbo(msg,{msg.text:match(terbo)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",terbo.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dterbo then
if ScriptFile.dterbo(msg) == false then
return false
end
...
if Plug.terbo then 
if msg.text and Plug.iterbo then
for k, terbo in pairs(Plug.terbo) do
local SearchText = msg.text:match(terbo)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(terbo..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(terbo..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iterbo(msg,{msg.text:match(terbo)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",terbo.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source terbo")
end 

end
else
print("The File Script.lua Not Runing in The Source terbo")
...
function tdcli_update_callback(data)
	...
	redis:srem(terbo..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE TER')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/terbo/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE TERBO\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس تربو بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(terbo..":SUDO_ID:")
redis:hset(terbo..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(terbo..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(terbo..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(terbo..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(terbo..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(terbo..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(terbo..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(terbo..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(terbo..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(terbo..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	print('¦'..msg.content_.ID..' : '..msg.sender_user_id_)
	msg.joinuser = true
end
	elseif msg.reply_terboup and  msg.reply_terboup.ID == "replyTerbiupInlineKeyboard" then
	msg.replyterboupInlineKeyboard = true
	end 
	input_inFo(msg)
	...
	if redis:get(terbo..'group:add'..data.chat_.id_) then
	redis:set(terbo..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(terbo..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(terbo..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(terbo..'group:name-100'..data.channel_.id_) or "")
	send_msg(SUDO_ID,"• قام شخص بطرد البوت من المجموعه الاتيه : \n• ألايدي : `-100"..data.channel_.id_.."`\n• الـمجموعه : "..Flter_terbodown(NameGroup).."\n\n• تـم مسح كل بيانات المجموعه بنـجاح ")
	...
	redis:mset(
	terbo..'group:name'..k,IDS.Title,
	terbo..'num_msg_terbo'..k,5,
	terbo..'group:add'..k,true,
	terbo..'lock_link'..k,true,
	terbo..'lock_spam'..k,true,
	terbo..'lock_webpage'..k,true,
	terbo..'lock_terbodown'..k,true,
	terbo..'lock_flood'..k,true,
	terbo..'lock_bots'..k,true,
	terbo..'mute_forward'..k,true,
	terbo..'mute_contact'..k,true,
	terbo..'mute_document'..k,true,
	terbo..'mute_inline'..k,true,
	terbo..'lock_username'..k,true,
	terbo..'replay'..k,true
	)
	redis:sadd(terbo..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(terbo..'username:'..ID,'username',user)
	redis:sadd(terbo..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(terbo..'username:'..ID,'username',user)
	redis:sadd(terbo..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(terbo..'username:'..ID,'username',user)
	redis:sadd(terbo..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(terbo..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(terbo..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(terbo..'username:'..data.user_.id_).username
	...
	redis:hset(terbo..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
